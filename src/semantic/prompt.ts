...
export function buildStoryMinerPrompt(bundle: SemanticEvidenceBundle): SemanticNarratorRequest {
  ...
  const system = zh
    ? [
        ...
        "beats 必须按真实时间顺序排列；attempt/workaround 应对应真实工具动作，correction/reversal 需要明确改口，success 需要可观察成功证据。",
        "如果证据不足，宁可不输出故事。",
    ...
    : [
        ...
        "Beats must follow real chronology. attempt/workaround should map to real tool actions, correction/reversal needs explicit reversal evidence, and success needs observable success evidence.",
