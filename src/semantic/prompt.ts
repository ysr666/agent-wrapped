...
export function buildStoryMinerPrompt(bundle: SemanticEvidenceBundle): SemanticNarratorRequest {
  ...
  const system = zh
    ? [
        ...
        "beats 必须按真实时间顺序排列；如果证据不足，宁可不输出故事。",
    ...
    : [
        ...
        "Every beat must cite event:* evidence only. momentHints may guide attention but are not factual beat evidence.",
        "Beats must follow real chronology. If evidence is weak, emit no story.",
...
export function buildNarrationPrompt(
  ...
  const system = zh
    ? [
        ...
        "persona 只能根据提供的确定性行为信号起一个‘本场角色’外号和一句 tagline；禁止输出 0-100 分或把它说成模型天生人格。",
    ...
    : [
        ...
        "Persona may only nickname this session from deterministic behavior signals. Never output 0-100 scores or imply an inherent model personality.",
