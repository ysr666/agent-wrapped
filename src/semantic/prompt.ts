...
export function buildStoryMinerPrompt(bundle: SemanticEvidenceBundle): SemanticNarratorRequest {
  ...
  const system = zh
    ? [
        ...
        "如果证据不足，宁可不输出故事。单纯的 tool failure→另一次 tool action 虽然可能真实，但通常只是工作流水；除非同一 story 还引用了 correction/reversal、user_pushback、capability_gap、breakdown 之一，或同一证据上有 false_dawn/plot_twist/boomerang/correction_arc 的 momentHint，否则不要把它输出成剧情。失败后的技术诊断 claim 不是剧情转折；若更早的 claim 真被后续证据打破，应使用 false_dawn/reversal。",
    ...
    : [
        ...
        "If evidence is weak, emit no story. A bare tool failure followed by another tool action may be true, but it is usually worklog rather than a highlight: do not output it as a story unless the same story also cites correction/reversal, user pushback, capability gap, or breakdown, or shares evidence with a false_dawn/plot_twist/boomerang/correction_arc momentHint. A technical diagnosis claim written after a failure is not a story turn; if an earlier claim is actually punctured by later evidence, use false_dawn/reversal.",
...
export function buildNarrationPrompt(
  ...
  const system = zh
    ? [
        ...
        "title/commentary 可以写时间顺序上的反差，但不能把一次用户纠正扩写成‘全靠用户才行动’‘用户不催就不干活’之类因果、绝对习惯或内心状态。没有可靠笑点就省略 commentary。",
    ...
    : [
        ...
        "Titles/commentary may punch up the chronological contrast, but must not turn one user correction into an invented sole cause, absolute habit, or inner state such as claiming the Agent acts only when pushed. Omit commentary when no grounded punchline is available.",
