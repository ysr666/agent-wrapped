  ...
  if (stories !== undefined && stories !== null) {
    ...
    stories.forEach((value, storyIndex) => {
      ...
      const beats = entry.beats.map((beatValue, beatIndex) => {
        ...
        if (typeof kind !== "string" || !BEAT_KINDS.has(kind as StoryBeatKind) || !evidenceIds) {
      ...
      candidates.push({ arcKind: arcKind as StoryArcKind, beats, confidence });
    });
  }
...
function failureCue(text: string | undefined): boolean {
  return !!text && /(?:失败|报错|不行|没修好|挂了|崩|错误|failed|error|broken|denied|blocked)/iu.test(text);
}

function beatCompatible(kind: StoryBeatKind, evidence: Array<SemanticEvidenceBundle["events"][number]>): boolean {
  if (kind === "setup" || kind === "claim") return evidence.some((event) => event.actor === "assistant" || event.actor === "user");
  if (kind === "attempt" || kind === "workaround") return evidence.some((event) => event.kind === "tool_call" || event.actor === "assistant");
  if (kind === "failure" || kind === "block") {
    return evidence.some((event) => event.kind === "tool_error" || (event.kind === "turn_end" && event.isError) || failureCue(event.text));
  }
  if (kind === "user_pushback") return evidence.some((event) => event.actor === "user");
  if (kind === "correction" || kind === "reversal" || kind === "recovery") return evidence.some((event) => event.actor === "assistant");
  if (kind === "success") return evidence.some((event) => (event.kind === "tool_result" && !event.isError) || event.actor === "assistant");
  return false;
}

function hasBefore(beats: SemanticStoryCandidate["beats"], left: StoryBeatKind[], right: StoryBeatKind[]): boolean {
...
function arcPatternValid(candidate: SemanticStoryCandidate): boolean {
  const beats = candidate.beats;
  switch (candidate.arcKind) {
    case "false_dawn":
      return hasBefore(beats, ["claim", "success"], ["failure", "block", "user_pushback", "correction", "reversal"]);
    case "failure_then_workaround":
    case "capability_gap_then_improvisation":
      return hasBefore(beats, ["failure", "block"], ["workaround", "attempt", "recovery", "success"]);
    case "mistake_then_correction":
    case "reversal":
      return hasBefore(beats, ["setup", "claim", "attempt"], ["correction", "reversal"]);
    case "user_pushback_then_recovery":
      return hasBefore(beats, ["user_pushback"], ["correction", "recovery", "attempt", "workaround", "success"]);
    case "breakdown_then_resume":
      return hasBefore(beats, ["failure", "block", "setup"], ["recovery", "attempt", "workaround", "success"]);
    case "other":
...
/**
 * Local structural validation. Existing evidence ids are not enough: beat order,
 * actor/event compatibility and arc shape must also hold before narration.
 */
export function validateStoryCandidates(
  ...
  candidates.forEach((candidate, candidateIndex) => {
    ...
    for (const beat of candidate.beats) {
      ...
      const concrete = evidence.filter((entry): entry is SemanticEvidenceBundle["events"][number] => entry !== undefined);
    ...
    if (!invalidReason && !arcPatternValid(candidate)) invalidReason = "arc-pattern-not-supported";
    if (invalidReason) {
